...
export const getStaticProps = async ({ locale }) => {
  ...
  const categoryLocal = 'cryptocurrency';
...
export default function Accessories({ categorie, entries, category, locale }) {
  ...
  return (
    <div style={{
      ...
      <SecondHeader />
      <HeadPage 
        title={category.titleShow}
        description={category.description}
        image={category.icon}
      />
      <ReviewOfert blogs={entries} article={locale == 'es' ? 'Criptomonedas' : 'Crytocurrrency'} cantidad={entries.length} />
